...
#define CATCH_RUNTIME_WITH_MSG catch(std::exception const& ex)		\
	...
	static void insert_button(HWND handle, int index, TBBUTTON const& btn, bool dropdown = false)
	{
		...
		if(dropdown) ::SendMessage(handle, TB_SETEXTENDEDSTYLE, 0, TBSTYLE_EX_DRAWDDARROWS);
